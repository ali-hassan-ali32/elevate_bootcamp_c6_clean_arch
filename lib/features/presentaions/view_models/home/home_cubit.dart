...
class HomeCubit extends Cubit<HomeState> {
  ...
  final StreamController<HomeUiEvent> _uiController = StreamController.broadcast();
  ...
  Future<void> _getProductsByCategoryId(GetProductsByCategoryIdEvent event) async {
    emit(state.copyWith(
      productResources: Resources.loading()
    ));
    ...
    switch(response) {
      case Success<List<ProductEntity>>():
        emit(state.copyWith(productResources: Resources.success(response.data)));
      case Failure<List<ProductEntity>>():
        _uiController.add(ShowMessageEvent(response.appError?.message ?? ''));

        emit(state.copyWith(productResources: Resources.failure(
  ...
  Future<void> _getCategories(GetCategoriesEvent event) async {
    emit(state.copyWith(
        categoryResources: Resources.loading()
    ));
    ...
    switch(response) {
      case Success<List<CategoryEntity>>():
        emit(state.copyWith(categoryResources: Resources.success(response.data)));

      case Failure<List<CategoryEntity>>():
        emit(state.copyWith(categoryResources: Resources.failure(
            response.appError?.message ?? 'Something Went Wrong',
        )));
    }
  }
  ...
  Future<void> _getBanners(GetBannersEvent event) async {
    ...
    switch(response) {
      case Success<List<BannerEntity>>(): {
        emit(state.copyWith(bannerResources: Resources.success(response.data)));
      }
      case Failure<List<BannerEntity>>():
        emit(state.copyWith(bannerResources: Resources.failure(
          response.appError?.message ?? 'Something Went Wrong',
        )));
        }
    }
