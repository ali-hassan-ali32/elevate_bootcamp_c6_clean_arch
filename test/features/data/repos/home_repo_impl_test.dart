...
void main() {
  ...
  group('Test Get Banners Function', () {
    test('Verify Call Function Correctly', () {
      when(
        homeLocalDatasource.getBanners(),
      ).thenAnswer((_) async => BannersResponse());
      when(
        homeMapper.mapBannersResponseToBannersEntityList(any),
      ).thenAnswer((_) => <BannerEntity>[]);
      homeRepo.getBanners();
    ...
    test('Test Get Banners return The Correct List', () async {
      var responseList = List.filled(
        3,
        BannerEntity(title: '', image: '', alignment: Alignment.bottomLeft),
      );
      when(
        homeLocalDatasource.getBanners(),
      ).thenAnswer((_) async => BannersResponse());
  ...
  group('Test Get Categories Function', () {
    test('Verify Call Function Correctly', () {
      when(
        homeApiClient.getCategories(),
      ).thenAnswer((_) async => CategoriesResponse());
      when(
        homeMapper.mapCategoryResponseListToCategoryEntityList(any),
      ).thenAnswer((_) => <CategoryEntity>[]);
